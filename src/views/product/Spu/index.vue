  ...
  <div>
    ...
    <el-card>
      <div v-show="scene==0">
        ...
        <el-pagination style="text-align: center;" :current-page="page" :page-size="limit" :page-sizes="[3,5,10]" :total="total" layout="prev,pager,next,jumper,->,sizes,total" @current-change="getSpuList" @size-change="handleSizeChange">
        </el-pagination>
      </div>
    ...
    <el-dialog :title="`${spu.spuName}的sku列表`" :visible.sync="dialogTableVisible" :before-close="close">
      <el-table :data="skuList" style="width: 100%;" border v-loading="loading">
        ...
        <el-table-column prop="weight" label="默认图片" width="width">
          <template slot-scope="{row,$index}">
...
export default {
  ...
  data() {
    return {
        ...
        // 分页器一个需要展示数据的条数
        total:0,
        // 0代表展示spu列表  1代表添加spu|修改spu  2代表添加sku
        scene:0,
        // 控制对话框的显示与隐藏
  ...
  methods: {
    ...
    getCategoryId({categoryId,level}){
      // categoryId:获取到一、二、三级分类的id  level:为了区别是几级id
      if(level==1){
