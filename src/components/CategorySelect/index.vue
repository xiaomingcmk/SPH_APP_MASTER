    ...
    <el-form :inline="true" class="demo-form-inline" :model="cForm">
      ...
      <el-form-item label="三级分类">
        <el-select placeholder="请选择" v-model="cForm.category3Id" @change="handler3" :disabled="show">
          <el-option :label="c3.name" :value="c3.id" v-for="(c3,index) in list2" :key="c3.id"></el-option>
        </el-select> 
      </el-form-item>
    </el-form>
  </div>
...
export default {
  ...
  methods: {
    ...
    // 一级分类的select事件回调（当一级分类的option发生变化是获取相应的二级分类的数据）
    async handler1(){
      // 清除数据
      this.list2=[]
    ...
    // 二级分类的select事件回调（当二级分类的option发生变化是获取相应的三级分类的数据）
    async handler2(){
      // 清除数据
      this.list3=[]
      this.cForm.category3Id=''
